...
JsValueRef getProperty(JsValueRef object, const wchar_t *propertyName)
{
	JsValueRef Property;
	JsPropertyIdRef PropertyId;
...
JsValueRef CALLBACK JSGetFile(JsValueRef callee, bool isConstructCall, JsValueRef *arguments, unsigned short argumentCount, void *callbackState)
{
	OutputDebugString(L"Enter JSGetFile\n");
	return JS_INVALID_REFERENCE;
}

JsValueRef CALLBACK JSAddFile(JsValueRef callee, bool isConstructCall, JsValueRef *arguments, unsigned short argumentCount, void *callbackState)
{
	OutputDebugString(L"Enter JSAddFile\n");
	JsValueRef		Args[2];
	JsValueRef		Result;

	JsErrorCode		nError{ JsNoError };
	JsPropertyIdRef FilesId, PushId;
	JsValueRef		Files, Push;
	JsValueType		Type;
	bool			UIUpdate = false;

	JsGetPropertyIdFromName(L"files", &FilesId);
	JsGetProperty(arguments[0], FilesId, &Files);
	nError = JsGetValueType(Files, &Type);

	JsGetPropertyIdFromName(L"push", &PushId);
	JsGetProperty(Files, PushId, &Push);
	nError = JsGetValueType(Push, &Type);

	Args[0] = Files;
	Args[1] = arguments[1];
	nError = JsCallFunction(Push, Args, 2, &Result);

	nError = JsGetValueType(arguments[2], &Type);
	JsBooleanToBool(arguments[2], &UIUpdate);

	if (true == UIUpdate) {
		CFileRenamerDlg *dlg = (CFileRenamerDlg *)AfxGetMainWnd();
		const wchar_t * pszPath;
		size_t			nSize;
		JsStringToPointer(arguments[1], &pszPath, &nSize);
		dlg->addFileToRightPane(pszPath);
	}

	return JS_INVALID_REFERENCE;
}

JsValueRef CALLBACK JSPrintAll(JsValueRef callee, bool isConstructCall, JsValueRef *arguments, unsigned short argumentCount, void *callbackState)
{
	OutputDebugString(L"Enter JSPrintAll\n");
	return JS_INVALID_REFERENCE;
}

JsValueRef CALLBACK JSFileListConstructor(JsValueRef callee, bool isConstructCall, JsValueRef *arguments, unsigned short argumentCount, void *callbackState)
{
	JsValueRef fileList = JS_INVALID_REFERENCE;
	JsValueRef files = JS_INVALID_REFERENCE;

	OutputDebugString(L"Enter JSFileListConstructor\n");

	JsCreateObject(&fileList);
	JsCreateArray(10, &files);
	setProperty(fileList, L"files", files);

	setCallback(fileList, L"addFile", JSAddFile, nullptr);
	setCallback(fileList, L"printAll", JSPrintAll, nullptr);
...
void CFileRenamerDlg::OnBnClickedRenameButton()
{
	...
	JsValueRef		Function;
	JsValueType		Function_Type;
	JsValueRef		Function_Value;
	void *			callbackState = NULL;
	...
	__try {
		...
		JsCreateFunction(JSFileListConstructor, nullptr, &jsConstructor);
		setProperty(GlobalObject, L"FileList", jsConstructor);

		JsValueRef Args[3];
		nError = JsPointerToString(L"self", wcslen(L"self"), &(Args[0]));	// 모든 함수의 첫번째 인자는 항상 thisArg다
		
		JsValueRef jsLeftPane, jsRightPane;
		JsConstructObject(jsConstructor, Args, 1, &jsLeftPane);
		setProperty(GlobalObject, L"leftPane", jsLeftPane);

		JsConstructObject(jsConstructor, Args, 1, &jsRightPane);
		setProperty(GlobalObject, L"rightPane", jsRightPane);
		...
		for (nIndex = 0; nIndex < m_nNumberOfFiles; nIndex++)
		{
			...
			JsBoolToBoolean(false, &UIUpdate);
			Args[2] = UIUpdate;
			
			nError = JsGetPropertyIdFromName(L"addFile", &Function);
			nError = JsGetProperty(jsLeftPane, Function, &Function_Value);
			nError = JsGetValueType(Function_Value, &Function_Type);
			nError = JsCallFunction(Function_Value, Args, 3, &Result);
		}

		nError = JsGetPropertyIdFromName(L"RenameAll", &Function);
		nError = JsGetProperty(GlobalObject, Function, &Function_Value);
		nError = JsGetValueType(Function_Value, &Function_Type);
		nError = JsCallFunction(Function_Value, Args, 1, &Result);
	}
	__finally {
...
void CFileRenamerDlg::addFileToRightPane(LPCWSTR pszFile)
{
	int nItem = 0;
	wchar_t szIndex[MAX_PATH];
	...
	wcscpy_s(m_FileList[nItem].szDstFile, MAX_PATH, pszFile);
	/////////////////////////////////////////////////////////////////////////////

	swprintf_s(szIndex, MAX_PATH, L"%d", nItem + 1);
	((CListCtrl *)GetDlgItem(IDC_TO_LIST))->InsertItem(nItem, szIndex, 0);

	_wsplitpath(pszFile, NULL, NULL, szFname, szExt);
